...
class PreviewManager {
  ...
  async startPreview(projectPath) {
    ...
    if (this._webviewArea) {
      this._webviewArea.innerHTML = ''
      if (!this._emptyEl) {
        this._emptyEl = document.createElement('div')
        this._emptyEl.className = 'preview-empty'
        this._emptyEl.innerHTML = '<div class="preview-empty-icon">\u{1F310}</div><div>Open a project to preview</div>'
    ...
    if (this._pathEl) this._pathEl.textContent = ''
    this._setFrameworkBadge('html')
  }
  async stopPreview() {
  ...
  _handleEvent(ev) {
    ...
    const styles = ev.styles || {}
    const computed = ev.computedStyles || {}
    const normalized = {
      ...ev,
      computedStyles: {
        color: styles.color ?? computed.color,
        backgroundColor: styles.backgroundColor ?? computed.backgroundColor,
        fontSize: styles.fontSize ?? computed.fontSize,
        fontFamily: styles.fontFamily ?? computed.fontFamily,
        fontWeight: styles.fontWeight ?? computed.fontWeight,
        lineHeight: styles.lineHeight ?? computed.lineHeight,
        display: styles.display ?? computed.display,
        position: styles.position ?? computed.position,
        padding: styles.padding ?? computed.padding,
        margin: styles.margin ?? computed.margin,
        border: styles.border ?? computed.border,
        borderRadius: styles.borderRadius ?? computed.borderRadius,
        boxShadow: styles.boxShadow ?? computed.boxShadow,
        opacity: styles.opacity ?? computed.opacity,
        width: styles.width ?? computed.width,
        height: styles.height ?? computed.height,
        textAlign: styles.textAlign ?? computed.textAlign,
        overflow: styles.overflow ?? computed.overflow
      },
      rect: ev.rect || null
    }
    if (ev.action === 'element-selected') {
      LiveFront.state.selectedElement = normalized
      LiveFront.EventBus.emit('element:selected', normalized)
      this._jumpToElement(normalized)
    } else if (ev.action === 'element-hovered') {
      LiveFront.state.hoverElement = normalized
      LiveFront.EventBus.emit('element:hovered', normalized)
